...
void MainWindow::actualizar1()
{
    ...
    for(int i=0;i<objetoo.size();i++){
        ...
        bordercollision(objetoo.at(i)->gettrono());//actualiza el choque con el borde
...
void MainWindow::felicidades()
{
    objetoo.push_back(new trono);
    objetoo[0] -> actualizar1(v_limit);
    objetoo[0] -> setFlag(QGraphicsItem::ItemIsFocusable);
    objetoo[0] -> setFocus();

    scene -> addItem(objetoo[0]);
}
...
void MainWindow::niveles(){
    ...
    if(n == 5){
        ...
        connect(timerr,SIGNAL(timeout()),this,SLOT(felicidades()));
...
void MainWindow::aunmnmentar()
{
     int v;
     QString datos;
     QFile file("Nuevo documento de texto (2).txt");           //Objeto para manejar la lectura del archivo
     ...
     if (datos == "1"){
      ...
      puntaje_+=v;                                                          //se restaun valor a vida
...
void MainWindow::aunmnmentar1()
{
     int v;
     QString datos;
     QFile file("Nuevo documento de texto (4).txt");           //Objeto para manejar la lectura del archivo
     file.open(QIODevice::ReadOnly);     //Abre el archiv en modo lectura
     datos=file.readLine();
     file.close();           //se cierra el archivo
     if (datos == ""){
     ...
     if (datos == "1"){
      ...
      puntaje2_+=v;                                                          //se restaun valor a vida
...
void MainWindow::decrementar()
{
    int v;
    QString datos;
    QFile file("enemigos.txt");           //Objeto para manejar la lectura del archivo
    file.open(QIODevice::ReadOnly);     //Abre el archiv en modo lectura
    datos=file.readLine();
    file.close();           //se cierra el archivo
    if (datos == ""){
...
void MainWindow::on_pushButton_4_clicked()      //para cargar la partida
{
    ...
    QList<QString> dats;
    QString nivel, vida, puntaje;
    QString info;
    QFile file("Nuevo documento de texto.txt");           //Objeto para manejar la lectura del archivo
    file.open(QIODevice::ReadOnly);     //Abre el archiv en modo lectura
    info=file.readLine();
    file.close();           //se cierra el archivo
    ...
    n=nivel.toInt();                                        //se cambia el nivel
    vida_=vida.toInt();                                    //vida cambia de valor
    puntaje_=puntaje.toInt();                                 //puntaje cambia su valor
